...
export function DeumahHeader() {
 ...
 return <header className="bg-deumah-navy-950 text-white"><div className="mx-auto flex h-16 max-w-7xl items-center gap-6 px-4 sm:px-6 lg:px-8">
  ...
  <div className="ms-auto flex items-center gap-2"><button className="hidden items-center gap-2 rounded-deumah-sm border border-white/15 px-3 py-2 text-sm md:flex"><PinIcon className="size-4"/>{activeCityName}</button><Link href={pathname} locale={locale==='ar'?'en':'ar'} className="rounded-deumah-sm border border-white/15 px-3 py-2 text-sm">{locale==='ar'?'English':'العربية'}</Link><button aria-label={t('favorites')} className="hidden rounded-full p-2 hover:bg-white/10 sm:block"><HeartIcon className="size-5"/></button><Link href="/login" className="hidden rounded-deumah-sm border border-white/20 px-4 py-2 text-sm md:block">{t('login')}</Link><Link href="/register" className="hidden rounded-deumah-sm bg-deumah-green-700 px-4 py-2 text-sm font-semibold md:block">{t('signup')}</Link><button aria-label="Menu" className="rounded-full p-2 hover:bg-white/10 lg:hidden"><MenuIcon className="size-6"/></button></div>
 </div></header>;
}
